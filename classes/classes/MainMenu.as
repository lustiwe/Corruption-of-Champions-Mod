package classes
{
	...
	public class MainMenu extends BaseContent
	{
		...
		private var mainMenuContent:Array = [];
		private var mainMenuConfigured:Boolean = false;
		...
		public function mainMenu():void
		{
			...
			if (!mainMenuConfigured) {
				...
				var button:CoCButton = mainView.getChildByName("mainmenu_button_0") as CoCButton;
				if (button != null) {
					if (player.str > 0) {
						button.alpha = 1;
						button.enabled = true;
						button.toolTipText = "Get back to gameplay?";
						button.callback = playerMenu;
					}
					else {
						button.alpha = 0.5;
						button.enabled = false;
						button.toolTipText = "Please start a new game or load an existing save file.";
		...
		private function configureNewButton():CoCButton {
			var button:CoCButton;
			var font:Font = new ButtonLabelFont();
			...
			var b:MovieClip = new buttonBackground0();
		...
		private function configureMainMenu():void {
			...
			var mainMenuButtons:Array = new Array(
				...
				["New Game", getGame().charCreation.newGameGo, "Start a new game."],
			...
			var cocLogo:MovieClip = new CoCLogo();
			cocLogo.height = 400;
			cocLogo.width = 600;
			cocLogo.x = Math.floor(getGame().stage.stageWidth / 2) - (cocLogo.width / 2);
			cocLogo.y = 1;
			mainMenuContent.push(cocLogo);
			var disclaimer:MovieClip = new Disclaimer();
			disclaimer.height = 90;
			disclaimer.width = 620;
			disclaimer.x = Math.floor(getGame().stage.stageWidth / 2) - (disclaimer.width / 2);
			disclaimer.y = Math.floor(getGame().stage.stageHeight / 2) + 80;
			mainMenuContent.push(disclaimer);
			...
			disclaimerInfo.height = disclaimer.height;
			disclaimerInfo.width = 540;
			disclaimerInfo.x = disclaimer.x + 80;
			disclaimerInfo.y = disclaimer.y + 0;
			...
			mainMenuContent.push(disclaimerInfo);
			...
			miniCredit.x = Math.floor(getGame().stage.stageWidth / 2) - (miniCredit.width / 2);
			miniCredit.y = Math.floor(getGame().stage.stageHeight / 2) + 10;
			miniCredit.defaultTextFormat = new TextFormat("Palatino Linotype, serif", 16, 0, null, null, null, null, null, "center", null, null, null, -2);
			miniCredit.htmlText = "Created by Fenoxo\n";
			...
			versionInfo.x = getGame().stage.stageWidth - versionInfo.width;
			versionInfo.y = getGame().stage.stageHeight - 80;
			versionInfo.selectable = false;
			versionInfo.defaultTextFormat = new TextFormat("Palatino Linotype, serif", 16, 0, true, null, null, null, null, "right");
			...
			mainMenuContent.push(versionInfo);
			var websiteInfo:TextField = new TextField();
			websiteInfo.name = "websiteInfo";
			websiteInfo.height = 40;
			websiteInfo.width = 200;
			websiteInfo.x = Math.floor(getGame().stage.stageWidth / 2) - (websiteInfo.width / 2);
			websiteInfo.y = Math.floor(getGame().stage.stageHeight / 2) + 260;
			websiteInfo.selectable = false;
			websiteInfo.defaultTextFormat = new TextFormat("Palatino Linotype, serif", 20, 0, true, null, null, "www.fenoxo.com", null, "center");
			websiteInfo.htmlText = "<a href=\"http://www.fenoxo.com/\">www.fenoxo.com</a>";
			mainMenuContent.push(websiteInfo);
			for (i = 0; i < mainMenuButtons.length; i++) {
				var button:CoCButton = configureNewButton();
				button.name = "mainmenu_button_" + i;
				button.width = BindDisplay.BUTTON_REAL_WIDTH;
				button.height = BindDisplay.BUTTON_REAL_HEIGHT;
				button.x = Math.floor(getGame().stage.stageWidth / 2) - 232 + ((i % 3) * 155);
				button.y = Math.floor(getGame().stage.stageHeight / 2) + 175 + (Math.floor(i / 3) * 45);
				button.labelField.htmlText = mainMenuButtons[i][0];
				button.callback = mainMenuButtons[i][1];
				button.toolTipText = mainMenuButtons[i][2];
				//button.toolTipHeader = button.labelText;
				mainMenuContent.push(button);
				if (i == 0) {
					if (player.str <= 0) {
						button.enabled = false;
						button.alpha = 0.5;
			...
			for (i = 0; i < mainMenuContent.length; i++) {
		...
		/*private function updateMainMenuTextColours():void {
			...
			for (var i:int = 0; i < elements.length; i++) {
				var fmt:TextFormat = new TextFormat();
				fmt.color = 0;
				var tf:TextField = mainView.getChildByName(elements[i]) as TextField;
		...
		public function showMainMenu():void {
			if (mainMenuConfigured) {
				for (var i:int = 0; i < mainMenuContent.length; i++) {
					mainMenuContent[i].visible = true;
				}
			}
			mainView.mainText.visible = false;
			mainView.scrollBar.visible = false;
		...
		public function hideMainMenu():void {
			if (mainMenuConfigured) {
				for (var i:int = 0; i < mainMenuContent.length; i++) {
					mainMenuContent[i].visible = false;
				}
			}
		...
		public function creditsScreen():void {
			...
			doNext(mainMenu);
			addButton(1, "Image Credits", imageCreditsScreen);
		}
		...
		public function imageCreditsScreen():void
		{
			hideMainMenu();
			clearOutput();
			outputText("<b><u>Image Credits</u></b>\n");
