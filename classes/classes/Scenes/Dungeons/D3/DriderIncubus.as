	...
	public class DriderIncubus extends AbstractSpiderMorph
	{
		...
		private function performLustAttack():void
		{
			...
			else
			{
				...
				if(player.findStatusAffect(StatusAffects.TaintedMind) < 0 && !this._seenResolute)
				{
		...
		private function spearStrike():void
		{
			...
			var damage:Number = (str + weaponAttack) * 0.4;
		...
		private function bite():void
		{
			...
			else
			{
				...
				else if(combatMiss() || combatFlexibility())
				{
		...
		private function kick():void
		{
			...
			{
				damage = str + weaponAttack + 25 - rand(player.tou);
				if(damage > 0)
				{
