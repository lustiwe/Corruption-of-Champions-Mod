package classes.Scenes.Dungeons.D3
{
	...
	public class LethiceScenes extends BaseContent
	{
		...
		private function woofwoof():void
		{
			...
			dynStats("lus=", 100, "int/", 5, "lib", 100, "sens", 50, "cor", 70, "resisted", false);
			menu();
		...
		private function killHerPrompt():void {
			if (player.weaponVerb == "slash" || player.weaponVerb == "cleave" || player.weaponVerb == "keen cut" || player.weaponVerb == "stab") {
				...
				addButton(0, "Snap Her Neck", killHer, 0);
				addButton(1, "Behead Her", killHer, 1);
				mainView.bottomButtons[0].toolTipText = "Snap her neck and make it quick.";
				mainView.bottomButtons[1].toolTipText = "Off with her head!\n\n(Warning: Contains gore!)";
			}
			else {
				killHer();
			}
		...
		private function postEndingChoices():void {
			...
			mainView.bottomButtons[0].toolTipText = "Export your save for possible use in CoC2.";
			if (flags[kFLAGS.GAME_END] != GAME_END_CONSORT) {
				addButton(1, "Continue", postEndingReturn);
				mainView.bottomButtons[1].toolTipText = "Export your save for possible use in CoC2.";
			}
			addButton(4, "Quit", quitToMenu);
			mainView.bottomButtons[4].toolTipText = "Quit and return to the main menu?";
		}
		...
		private function postEndingReturn():void {
			...
			outputText("\n\n<b>At this point, this is where the canon ends.</b>");
			getGame().inDungeon = false;
			getGame().inRoomedDungeon = false;
